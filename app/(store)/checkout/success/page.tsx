...
export default function CheckoutSuccessPage() {
  ...
  return (
    <div className="flex min-h-[75vh] flex-col items-center justify-center px-4 py-16 text-center" style={{ background: "#FAFAFA" }}>
      <div className="mx-auto flex w-full max-w-2xl flex-col items-center">
        ...
        <p className="mt-3 text-lg text-[#6B7280]">Thank you{data?.firstName ? `, ${data.firstName}` : ""}.</p>
        ...
        {data?.lines?.length ? (
          <div className="mt-8 flex flex-wrap justify-center gap-2 sm:gap-3">
            {data.lines.map((line, i) => (
              <div key={i} className="h-14 w-14 overflow-hidden rounded-lg border border-[#E5E7EB] bg-[#F3F4F6] sm:h-16 sm:w-16">
                {line.image ? (
        ...
        <div className="mt-10 flex w-full max-w-md flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <Link href="/shop" className="flex-1">
            <Button className="h-12 w-full rounded-full bg-[#0F172A] text-white hover:opacity-90">Continue Shopping</Button>
          </Link>
        </div>
